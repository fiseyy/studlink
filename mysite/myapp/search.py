...
top_universities_ru = [
    ...
    "Калининградский федеральный университет им. Канта",
    ...
    "Магаданский государственный университет",
    ...
    "Государственный морской университет им. адмирала Ушакова",
    ...
    "Белгородский технологический университет",
...
class Search:
    ...
    def __init__(self, user):
        self.user = user
        self.user_tokens = self._user_tokens()
    ...
    def apply_filters(self, queryset, direction=None, profession=None, city=None):
        if direction and direction in job_filters:
            ...
            queryset = queryset.filter(title__in=professions)

        if profession:
            queryset = queryset.filter(title=profession)
    ...
    def search_vacancies(
        ...
        vacancies = self.apply_filters(vacancies, direction, profession, city)

        # Фильтры
        if schedule:
            vacancies = vacancies.filter(schedule=schedule)
        ...
        for vacancy in vacancies:
            v_tokens = self.vacancy_tokens(vacancy)
            score = self.similarity(self.user_tokens, v_tokens)
    ...
    def search_freelance(self, direction=None, profession=None, city=None, limit=20):
        freelances = Vacancy.objects.all()  # потом заменишь на Freelance model
        freelances = self.apply_filters(freelances, direction, profession, city)
        return freelances[:limit]

    # Проекты
    def search_projects(self, direction=None, profession=None, city=None, limit=20):
        projects = Vacancy.objects.all()  # потом заменишь на Project model
        projects = self.apply_filters(projects, direction, profession, city)
        return projects[:limit]

    # Резюме
    def search_resumes(self, direction=None, profession=None, city=None, limit=20):
        resumes = UserProfile.objects.all()
        # позже фильтр по университетам
...
def search(request):
    ...
    if tab == "freelance":
        ...
        results = engine.search_resumes(direction, profession, city)

    else:
        results = engine.search_vacancies(
            direction,
            profession,
            city,
            schedule,
